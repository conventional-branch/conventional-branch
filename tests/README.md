...
2. **Examples** — every case in [`../data/examples.yaml`](../data/examples.yaml),
   which all eleven examples tables are rendered from, gets the verdict the regex
   actually gives it, and every page still renders the table and the grammar. The
   tables can no longer disagree with each other, so only the data needs checking.
...
8. **Grammar** — `spec.json`'s own `abnf` field offers exactly the types and
   trunk branches `spec.json` declares. The grammar shown on the pages is
   generated from those fields and cannot drift; the `abnf` string is written by
   hand and published to every tool that reads the spec, so it can, and this is
   what stops it.
