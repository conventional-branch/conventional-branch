...
- **[`llms.txt`](static/llms.txt)** ([hosted](https://conventionalbranch.org/llms.txt)) — an [llmstxt.org](https://llmstxt.org) index for language models: the prefixes, the rules and the validation regex inline, plus where to find everything else. The site publishes the same specification across eleven languages and an archived 1.0.0, and this is the file that says which one is current.
