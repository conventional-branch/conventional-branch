...
- **`ai/`**: Generic prefix for any AI coding agent (e.g., `ai/refactor-auth-flow`)
- **`copilot/`**: GitHub Copilot (e.g., `copilot/add-login-page`)
- **`cursor/`**: Cursor (e.g., `cursor/fix-header-bug`)
- **`claude/`**: Claude Code by Anthropic (e.g., `claude/security-patch`)
- **`codex/`**: OpenAI Codex (e.g., `codex/optimize-query`)
