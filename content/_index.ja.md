...
v1.1.0 の主な新機能は **AI エージェントソースプレフィックス（AI Agent Source Prefixes）** の導入です。これにより、AI コーディングエージェント（例：`ai/`、`copilot/`、`cursor/`、`claude/`、`codex/`）によって作成されたブランチを識別できるようになります。既存の v1.0.0 ブランチ名はすべて引き続き有効です。また、バージョン管理されたウェブサイトにバージョン切り替え機能が追加され、v1.0.0 と v1.1.0 の両方の仕様を参照できます。
